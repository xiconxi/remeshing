...
private:
    ...
    VariationalRemeshing remesh_;
};

Viewer::Viewer(const char* title, int width, int height)
    : MeshViewer(title, width, height), param_(mesh_), remesh_(mesh_)
{
}
...
//        }
        ...
        {
            remesh_.harmonic_parameterization();
            remesh_.initial_sampling(mesh_.n_vertices());
            remesh_.wire_merged_lloyd_relaxation(50);
//            remesh_.wire_merged_lloyd_relaxation(1);
//            remesh_.lloyd_relaxation(50);
//            set_draw_mode("Texture");
//            update_mesh();
        }
    }
