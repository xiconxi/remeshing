...
#define RuntimeError(x) x
#define NotImplementedError(x) x
...
namespace TriangleWrapperHelper {
    ...
    typedef std::unordered_map<Duplet, bool, HashFunc<Duplet> > BoundaryMarker;
    ...
    Regions extract_regions(const MatrixIr& faces,
        ...
        for (size_t i=0; i<num_edges; i++) {
            Duplet edge(edges(i, 0), edges(i, 1));
...
void TriangleWrapper::run_triangle(const std::string& flags) {
    ...
    if (num_segments > 0) {
        ...
        if (m_segment_markers.size() == 0)
            for (size_t i=0; i<num_segments; i++)
                in.segmentmarkerlist[i] = REGION_BOUNDARY;
        else {
            for (size_t i=0; i<num_segments; i++)
                in.segmentmarkerlist[i] = m_segment_markers[i];
        }
    }
